...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "swift-kie-server-springboot-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name swift-kie-server-springboot-service apps/swift-kie-server-springboot-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app swift-kie-server-springboot-service:1.0-SNAPSHOT
    oc expose svc/swift-kie-server-springboot-service
else

	echo "Launching the application locally..."
	pattern="swift-kie-server-springboot-service"
	files=( $pattern )
	cd ${files[0]}
