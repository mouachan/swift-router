...
public class MyRemoteServiceTest {
  ...
  /*  @Inject
    @RestClient
    SwiftRemoteService myRemoteService;

    @Test
    public void testExtensionsRestClientEndpoint() {
        Set<SwiftRemoteService.Extension> restClientExtensions = myRemoteService.getExtensionsById("io.quarkus:quarkus-rest-client");

        Assertions.assertEquals(1, restClientExtensions.size());
        for (SwiftRemoteService.Extension extension : restClientExtensions) {
